...
CREATE INDEX idx_wallet_auth_nonces_lookup ON wallet_auth_nonces (wallet_address, nonce);

ALTER TABLE wallet_auth_nonces ENABLE ROW LEVEL SECURITY;

-- Nonces are server-side only; no client access
...
CREATE INDEX idx_wallet_auth_events_address ON wallet_auth_events (wallet_address);
CREATE INDEX idx_wallet_auth_events_user_id ON wallet_auth_events (user_id);
